...
-- Verifica se um executavel existe
...
function M.find_root_dir(bufnr, markers)
  local success, root = pcall(vim.fs.root, bufnr, markers)
  ...
  logger.warn(string.format("Root dir nao encontrado, usando: %s", fallback))
  return fallback
end

-- Verifica se LSP ja esta ativo no buffer
...
function M.stop_buffer_clients(bufnr)
  ...
  for _, client in ipairs(clients) do
    logger.info(string.format("Parando LSP: %s", client.name))
    vim.lsp.stop_client(client.id)
  end
end

-- Formata documento usando LSP
function M.format_document()
  local bufnr = vim.api.nvim_get_current_buf()
  local clients = vim.lsp.get_clients({ bufnr = bufnr })

  if #clients == 0 then
    logger.warn("Nenhum LSP client disponivel para formatacao", true)
    return
  end
